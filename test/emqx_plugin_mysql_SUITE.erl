...
-module(emqx_plugin_mysql_SUITE).

-compile(export_all).

-define(PID, emqx_plugin_mysql).
...
init_per_suite(Config) ->
  emqx_ct_helpers:start_apps([emqx, emqx_plugin_mysql], fun set_special_configs/1),
  Config.

end_per_suite(_Config) ->
  emqx_ct_helpers:stop_apps([emqx_plugin_mysql, emqx]).
...
set_cmd(Key) ->
  emqx_cli_config:run(["config", "set", string:join(["mysql", Key], "."), "--app=emqx_plugin_mysql"]).
